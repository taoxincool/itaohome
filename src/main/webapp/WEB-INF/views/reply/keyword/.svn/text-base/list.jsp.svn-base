...
<div data-role="page">
    <div data-role="header" data-theme="b">
        ...
        <h1>关键字回复列表</h1>
        <a href="<c:url value="/reply/keyword/new"/> " data-role="button" data-icon="plus">新增</a>
    </div>
    <div data-role="content">
        <ul data-role="listview" data-inset="true" data-filter="true" data-filter-placeholder="搜索">
            <c:forEach items="${list}" var="item">
                <li>
                    <a href="<c:url value="/reply/keyword/view?id=${item.id}"/> ">
                        <h2>${empty item.inputContent ? '关注操作' : item.inputContent}</h2>
                        <p>回复内容：<c:out value="${item.replyContent}"/></p>
